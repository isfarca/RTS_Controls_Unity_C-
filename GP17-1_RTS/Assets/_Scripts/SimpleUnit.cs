...
{
    [SerializeField]
    private GameObject m_cBuilding_Barracks;
    ...
    private void Update()
    {
        ...
        else if(Input.GetKeyDown(KeyCode.Mouse0))
        {
            ...
            if (Physics.Raycast(m_cRay, out m_cInfo))
            {
                if (m_cInfo.collider.CompareTag("Ground"))
                {
                    GameObject cObj = Instantiate<GameObject>(m_cBuilding_Barracks, m_cInfo.point + new Vector3(0.0f, 1.25f, 0.0f), Quaternion.identity);
                }
            }
        }
    }
